...
class AreasController extends Controller
{
    ...
    public function agregar(GuardarArea $peticion)
    {
        ...
        $tipo = "success";
        if (!$exitoso) {
            $mensaje = "Error agregando proveedor. Intente más tarde";
            $tipo = "danger";
        }
    ...
    public function guardarCambios(GuardarCambiosDeArea $peticion)
    {
        ...
        $area->contacto = $peticion->input("contacto");
        

        $area->save();
        return redirect()->route("areas")->with(["mensaje" => "Proveedor editado correctamente", "tipo" => "success"]);
